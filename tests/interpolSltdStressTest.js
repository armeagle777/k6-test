...
import { buildSltdSearchRequest } from '../lib/sltdRequestFactory.js';

const API_URL = __ENV.API_URL || 'http://localhost:3000/api/interpol/sltd/search';
const AUTH_TOKEN = __ENV.AUTH_TOKEN || '';
const ENABLE_DEBUG = (__ENV.ENABLE_DEBUG || 'false').toLowerCase() === 'true';

const searchDuration = new Trend('sltd_search_duration', true);
const soapDuration = new Trend('sltd_soap_duration', true);
const hitCount = new Counter('sltd_hit_count');
const noHitCount = new Counter('sltd_no_hit_count');
const errors = new Counter('sltd_errors');
const totalRequests = new Counter('sltd_total_requests');

const rtLt500 = new Counter('sltd_rt_bucket_lt_500');
const rt500to1000 = new Counter('sltd_rt_bucket_500_1000');
const rt1000to1500 = new Counter('sltd_rt_bucket_1000_1500');
const rt1500to2000 = new Counter('sltd_rt_bucket_1500_2000');
const rt2000to2500 = new Counter('sltd_rt_bucket_2000_2500');
const rt2500to3000 = new Counter('sltd_rt_bucket_2500_3000');
const rtGt3000 = new Counter('sltd_rt_bucket_gt_3000');
...
export function setup() {
  // Use: k6 run --log-format raw --console-output logs/interpol_sltd_test_results.csv tests/interpolSltdStressTest.js
...
function createRequestId(terminalId) {
  return `sltd-${terminalId}-${exec.vu.idInTest}-${exec.vu.iterationInScenario}-${Date.now()}`;
}

function runTerminal() {
  const terminalId = normalizeTerminalId();
  const { payload, hitOrNoHit } = buildSltdSearchRequest();
...
export function terminal_1() { runTerminal(); }
export function terminal_2() { runTerminal(); }
export function terminal_3() { runTerminal(); }
export function terminal_4() { runTerminal(); }
export function terminal_5() { runTerminal(); }
export function terminal_6() { runTerminal(); }
export function terminal_7() { runTerminal(); }
...
export function handleSummary(data) {
  const total = metricCount(data, 'sltd_total_requests');
  const errorCount = metricCount(data, 'sltd_errors');
  const errorRate = total > 0 ? ((errorCount / total) * 100).toFixed(2) : '0.00';

  const buckets = [
    ['<500 ms', metricCount(data, 'sltd_rt_bucket_lt_500')],
    ['500-1000 ms', metricCount(data, 'sltd_rt_bucket_500_1000')],
    ['1000-1500 ms', metricCount(data, 'sltd_rt_bucket_1000_1500')],
    ['1500-2000 ms', metricCount(data, 'sltd_rt_bucket_1500_2000')],
    ['2000-2500 ms', metricCount(data, 'sltd_rt_bucket_2000_2500')],
    ['2500-3000 ms', metricCount(data, 'sltd_rt_bucket_2500_3000')],
    ['>3000 ms', metricCount(data, 'sltd_rt_bucket_gt_3000')],
  ];

  const lines = [];
  lines.push('INTERPOL SLTD /api/interpol/sltd/search - Stress Test Summary');
  lines.push('==============================================================');
  lines.push(`Total requests: ${total}`);
  lines.push(`Error rate: ${errorRate}% (${errorCount}/${total})`);
  lines.push(`Min latency: ${safeTrendValue(data, 'sltd_search_duration', 'min').toFixed(2)} ms`);
  lines.push(`Max latency: ${safeTrendValue(data, 'sltd_search_duration', 'max').toFixed(2)} ms`);
  lines.push(`Average latency: ${safeTrendValue(data, 'sltd_search_duration', 'avg').toFixed(2)} ms`);
  lines.push(`P90 latency: ${safeTrendValue(data, 'sltd_search_duration', 'p(90)').toFixed(2)} ms`);
  lines.push(`P95 latency: ${safeTrendValue(data, 'sltd_search_duration', 'p(95)').toFixed(2)} ms`);
  lines.push(`P99 latency: ${safeTrendValue(data, 'sltd_search_duration', 'p(99)').toFixed(2)} ms`);
  lines.push('');
  ...
  return {
    stdout: summaryText,
    'logs/sltd_summary.txt': summaryText,
  };
}
